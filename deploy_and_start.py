...
def register_rsu_in_ddns(ssh_client):
    """Obtém o IPv6 da RSU (wwan0) via SSH e o registra no DDNS (modo IPv6)."""
    try:
        log("Consulting wwan0 IPv6 address on RSU...", CYAN)
        stdin, stdout, stderr = ssh_client.exec_command("ip -6 addr show dev wwan0 scope global | grep inet6 | awk '{print $2}' | cut -d/ -f1 | head -n 1")
        ipv6 = stdout.read().decode('utf-8').strip()
        
        if not ipv6:
            log("No global IPv6 found on wwan0!", RED)
            return False
            
        log(f"RSU IPv6 discovered: {ipv6}", GREEN)
        
        url = f"{DDNS_URL}/update"
        payload = {"name": DDNS_RSU_NAME, "ip": ipv6}
        
        log(f"Registering in DDNS: {url}", CYAN)
        resp = requests.post(url, json=payload, timeout=5)
        if resp.status_code == 200:
            log("Successfully registered in DDNS!", GREEN)
            return True
        else:
            log(f"Failed to register in DDNS: HTTP {resp.status_code}", RED)
            return False
            
    except Exception as e:
        log(f"Exception during DDNS registration: {str(e)}", RED)
        return False

def get_rsu_ip_from_ddns():
    """Consulta o DDNS para encontrar o IPv6 atual da RSU."""
    try:
        url = f"{DDNS_URL}/resolve/{DDNS_RSU_NAME}"
        resp = requests.get(url, timeout=5)
        if resp.status_code == 200:
            data = resp.json()
            if "ip" in data:
                return data["ip"]
        log(f"Failed to resolve RSU IP from DDNS (HTTP {resp.status_code})", RED)
    except Exception as e:
        log(f"Error querying DDNS: {str(e)}", RED)
    return None
